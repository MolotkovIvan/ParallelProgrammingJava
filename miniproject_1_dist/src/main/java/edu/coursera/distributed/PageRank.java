...
//import edu.coursera.distributed.Website;
...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
    	...
    	JavaPairRDD<Integer, Double> newRanks = sites.join(ranks)
    			.flatMapToPair(kv -> {
    				...
    				while (iter.hasNext()) {
    					...
    					contribs.add(new Tuple2<Integer, Double>(target, currentRank / (double) edges.getNEdges()));
    				}
    	...
    	return newRanks.reduceByKey((Double d1, Double d2) -> d1 + d2)
